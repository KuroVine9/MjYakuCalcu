...
object MjScoreUtil {
    ...
    fun getRonScore(fuuToHan: MjFuuToHanVo, isOya: Boolean): MjScoreVo<MjScoreI.Ron> {
        ...
        return when (MjScore.ofHan(han)) {
            ...
            MjScore.MANKAN -> 8000
            MjScore.ELSE -> {
                val (ronScore, _) = scoreTable[fuu]?.get(han - 1) ?: throw IllegalStateException("Unsupport fuu: han=$han to fuu=$fuu")
    ...
    fun getTsumoScore(fuuToHan: MjFuuToHanVo): MjScoreVo<MjScoreI.Tsumo> {
        ...
        return when (scoreEnum) {
            ...
            MjScore.ELSE -> {
                val (_, tsumoScore) = scoreTable[fuu]?.get(han - 1) ?: throw IllegalStateException("Unsupport fuu: han=$han to fuu=$fuu")
                tsumoScore
            }
